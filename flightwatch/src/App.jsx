...
function PriceCalendar({ from, to, selectedDepart, selectedReturn, onSelectDepart, onSelectReturn, selectingReturn }) {
  ...
  const maxP = vals.length ? Math.max(...vals) : 999;
  const midP = (minP+maxP)/2;
  const priceColor = p => !p ? "#555E73" : p <= minP+(maxP-minP)*.25 ? "#34D399" : p <= midP ? "#FBBF24" : "#F87171";
  ...
  const handleDay = key => {
    if (selectingReturn) { if (selectedDepart && key <= selectedDepart) return; onSelectReturn(key); }
    else { onSelectDepart(key); }
  };
  const isSelected = k => k===selectedDepart||k===selectedReturn;
  const isInRange  = k => selectedDepart&&selectedReturn&&k>selectedDepart&&k<selectedReturn;
  const isStart    = k => k===selectedDepart;
  const isEnd      = k => k===selectedReturn;
  ...
  return (
    <div style={{background:"#111318",borderRadius:12,overflow:"hidden",border:"1px solid #252B3B"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 14px",borderBottom:"1px solid #252B3B"}}>
        <button onClick={prevMonth} style={{background:"#1E2330",border:"1px solid #2E3448",borderRadius:7,padding:"4px 8px",color:"#8B93A8",cursor:"pointer",display:"flex"}}><ChevLeft/></button>
        <div style={{fontSize:13,fontWeight:700,color:"#F0F2F8",letterSpacing:-0.3}}>{MONTHS[viewMonth.month]} {viewMonth.year}</div>
        <button onClick={nextMonth} style={{background:"#1E2330",border:"1px solid #2E3448",borderRadius:7,padding:"4px 8px",color:"#8B93A8",cursor:"pointer",display:"flex"}}><ChevRight/></button>
      </div>
      <div style={{padding:"6px 14px 4px",fontSize:10,fontWeight:600,color:selectingReturn?"#A78BFA":"#4F8EF7",textTransform:"uppercase",letterSpacing:.6}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",padding:"4px 8px 0"}}>
        {DAYS.map(d=><div key={d} style={{textAlign:"center",fontSize:9,fontWeight:600,color:"#555E73",padding:"4px 0",textTransform:"uppercase",letterSpacing:.4}}>{d}</div>)}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",padding:"2px 8px 10px",gap:"2px 0"}}>
        ...
        {Array.from({length:daysCount}).map((_,i)=>{
          ...
          const p=prices[key]; const past=isPast(key); const sel=isSelected(key);
          const rng=isInRange(key); const st=isStart(key); const en=isEnd(key); const chp=cheapDays.includes(key);
          return (
            <div key={key} onClick={()=>!past&&p&&handleDay(key)}
              style={{position:"relative",display:"flex",flexDirection:"column",alignItems:"center",padding:"4px 2px",
              ...
              <div style={{fontSize:11,fontWeight:sel?800:600,color:sel?"#fff":"#C8CEDF",lineHeight:1,marginBottom:2}}>{day}</div>
              {p&&<div style={{fontSize:8,fontWeight:700,color:sel?"rgba(255,255,255,.8)":priceColor(p),lineHeight:1}}>${p}</div>}
              {chp&&!sel&&<div style={{position:"absolute",top:1,right:2,width:4,height:4,borderRadius:"50%",background:"#34D399"}}/>}
            </div>
          );
        })}
      </div>
      <div style={{display:"flex",alignItems:"center",gap:12,padding:"8px 14px",borderTop:"1px solid #252B3B",justifyContent:"center"}}>
        {[["#34D399","Cheapest"],["#FBBF24","Moderate"],["#F87171","Expensive"]].map(([c,l])=>(
          <div key={l} style={{display:"flex",alignItems:"center",gap:4}}>
            <div style={{width:6,height:6,borderRadius:"50%",background:c}}/><span style={{fontSize:9,color:"#555E73",fontWeight:500}}>{l}</span>
          </div>
        ))}
      </div>
    </div>
...
function SwipeRow({ onDelete, children }) {
  ...
  const onTouchEnd   = () => {
    setSwiping(false);
    setOffset(o => o < -DELETE_W/2 ? -DELETE_W : 0);
    startX.current = null;
  };
  ...
  return (
    <div style={{position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"absolute",right:0,top:0,bottom:0,width:DELETE_W,
        background:"#F87171",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}
        onClick={()=>{setOffset(0);onDelete();}}>
        <TrashIcon/>
        <span style={{color:"#fff",fontSize:11,fontWeight:700,marginLeft:4}}>Delete</span>
      </div>
      {/* Content slides left */}
      <div onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}
        style={{transform:`translateX(${offset}px)`,transition:swiping?"none":"transform .25s ease",
...
// ── ADD/EDIT ROUTE FORM ────────────────────────────────────────────────────────
function RouteForm({ existing, onSave, onClose }) {
  ...
  const [flex,      setFlex]      = useState(existing?.flex   ?? 3);   // ±days
  const [threshold, setThreshold] = useState(existing?.threshold || "");
  const [pax,       setPax]       = useState(existing?.passengers || 1);
  const [picker,    setPicker]    = useState(null);
  const [selRet,    setSelRet]    = useState(false);
  const [step,      setStep]      = useState(isEdit ? "details" : "route");

  const canShowCal = from.code && to.code;
  const valid      = from.code && to.code && depart && threshold !== "" && Number(threshold) > 0;

  const handleSave = () => {
    if (!valid) return;
    const prices   = genCalPrices(from.code, to.code, new Date().getFullYear(), new Date().getMonth());
    const mockPrice= depart && prices[depart] ? prices[depart] : Number(threshold)+Math.round(Math.random()*60+10);
    const hist     = existing?.history || Array.from({length:9},()=>Math.round(mockPrice+(Math.random()-.4)*30));
    onSave({
      id: existing?.id || uid(),
      from:from.code, fromCity:from.city, to:to.code, toCity:to.city,
      depart, ret, flex, threshold:Number(threshold), passengers:pax,
      currentPrice: existing?.currentPrice ?? mockPrice, history:hist,
  ...
  if (picker) return (
    <AirportSheet value={picker==="from"?from.code:to.code}
      onSelect={a=>picker==="from"?setFrom(a):setTo(a)} onClose={()=>setPicker(null)}/>
  );

  return (
    <Sheet onClose={onClose} title={isEdit?"Edit Route":"Add Route"}>
      {/* Step indicator — hide for edit mode going straight to details */}
      {!isEdit && (
      ...
      {step==="details"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{background:"#1E2330",borderRadius:10,padding:"12px 14px"}}>
            <div style={{fontSize:14,fontWeight:800,color:"#F0F2F8",letterSpacing:-0.5,marginBottom:8}}>
              {from.code} → {to.code}
            </div>
            ...
            {!isEdit&&(
              <button onClick={()=>setStep("cal")}
                style={{fontSize:11,color:"#4F8EF7",background:"none",border:"none",
          ...
          {/* ±Flex days */}
          <div>
            <Lbl>Date Flexibility</Lbl>
            <div style={{display:"flex",gap:8}}>
              {[0,1,2,3].map(n=>(
                <button key={n} onClick={()=>setFlex(n)}
                  style={{flex:1,padding:"9px 0",borderRadius:8,fontFamily:"Inter,sans-serif",
                    background:flex===n?"rgba(79,142,247,.15)":"#1E2330",
            ...
            {flex>0&&depart&&(
              <div style={{fontSize:11,color:"#555E73",marginTop:6}}>
                Alert covers {fmt(addDays(depart,-flex))} – {fmt(addDays(depart,flex))}
          ...
          {(()=>{
            ...
            const vals=Object.values(prices);
            const minP=Math.min(...vals), avgP=Math.round(vals.reduce((a,b)=>a+b,0)/vals.length);
            return (
              <div style={{background:"rgba(79,142,247,.08)",border:"1px solid rgba(79,142,247,.2)",borderRadius:10,padding:"12px 14px"}}>
                <div style={{fontSize:10,color:"#4F8EF7",fontWeight:600,textTransform:"uppercase",letterSpacing:.6,marginBottom:6}}>Price intel for {fmt(depart)}</div>
                <div style={{display:"flex",gap:16}}>
                  <div><div style={{fontSize:22,fontWeight:800,color:"#F0F2F8",letterSpacing:-1}}>${cal}</div><div style={{fontSize:10,color:"#555E73"}}>This date</div></div>
                  <div><div style={{fontSize:22,fontWeight:800,color:"#34D399",letterSpacing:-1}}>${minP}</div><div style={{fontSize:10,color:"#555E73"}}>Cheapest</div></div>
                  <div><div style={{fontSize:22,fontWeight:800,color:"#8B93A8",letterSpacing:-1}}>${avgP}</div><div style={{fontSize:10,color:"#555E73"}}>Avg</div></div>
                </div>
              </div>
            );
          })()}

          {/* Threshold — blank placeholder, no pre-filled value */}
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",background:"#1E2330",
              ...
              <input type="number" value={threshold}
                ...
                style={{flex:1,background:"transparent",border:"none",fontSize:26,fontWeight:800,
                  ...
                  "::placeholder":{color:"#555E73"}}}/>
            </div>
            {(()=>{
              const prices=genCalPrices(from.code,to.code,new Date().getFullYear(),new Date().getMonth());
              const cal=prices[depart]; if(!cal||!threshold) return null;
              const diff=cal-Number(threshold);
              return (
                <div style={{fontSize:11,color:diff>0?"#34D399":"#FBBF24",marginTop:5,fontWeight:600}}>
                  {diff>0?`↓ $${diff} below today's price — you'd get notified now`:`↑ Above today's price of $${cal} — watching for a drop`}
                </div>
              );
            })()}
          </div>

          <PrimaryBtn onClick={handleSave} disabled={!valid}>
...
function AddTripForm({ onSave, onClose }) {
  ...
  return (
    <Sheet onClose={onClose} title="New Trip">
      ...
      <PrimaryBtn onClick={()=>{if(name.trim()){onSave({id:uid(),name:name.trim(),color,routes:[]});onClose();}}} disabled={!name.trim()}>
        Create Trip
      </PrimaryBtn>
    </Sheet>
  );
...
function Spark({ data, threshold, depart }) {
  ...
  // Build date labels: 14 days back from depart
  const endDate   = depart ? new Date(depart+"T12:00:00") : new Date();
  const startDate = new Date(endDate); startDate.setDate(startDate.getDate()-(data.length-1));
  const startLbl  = `${MONTHS_SHORT[startDate.getMonth()]} ${startDate.getDate()}`;
  const endLbl    = `${MONTHS_SHORT[endDate.getMonth()]} ${endDate.getDate()}`;
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",width:W}}>
        <span style={{fontSize:8,color:"#555E73"}}>{startLbl}</span>
        <span style={{fontSize:8,color:"#555E73"}}>{endLbl}</span>
      </div>
    </div>
  );
}

// ── FULL CHART with date labels ────────────────────────────────────────────────
function FullChart({ data, threshold, depart }) {
  ...
  // Date labels across bottom
  const endDate   = depart ? new Date(depart+"T12:00:00") : new Date();
  const labels    = data.map((_,i)=>{
    const d=new Date(endDate); d.setDate(d.getDate()-(data.length-1-i));
        ...
        <div style={{background:"#161A22",border:"1px solid #252B3B",borderRadius:14,padding:16}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
            <div>
              ...
              <div style={{fontSize:12,color:"#555E73",marginTop:4}}>
                {route.passengers} pax · {fmt(route.depart)}{route.ret?` – ${fmt(route.ret)}` : ""}
...
function HomeScreen({ trips, setTrips }) {
  ...
  const [detail,      setDetail]      = useState(null); // {tripId,routeId}
  const [editRoute,   setEditRoute]   = useState(null); // route object being edited
  ...
  if (detail) {
    ...
    if (route) return (
      <>
        <RouteDetail route={route} tripColor={trip.color}
      ...
      <div style={{padding:"16px 20px",display:"flex",flexDirection:"column",gap:14}}>
        ...
        {dropped.map(r=>(
          <div key={r.id} onClick={()=>setDetail({tripId:r.tripId,routeId:r.id})}
            style={{background:"linear-gradient(135deg,rgba(52,211,153,.12),rgba(52,211,153,.05))",
              border:"1px solid rgba(52,211,153,.3)",borderRadius:14,padding:"14px 16px",
        ...
        {trips.map(trip=>(
          <div key={trip.id} style={{background:"#161A22",border:"1px solid #252B3B",borderRadius:14,overflow:"hidden"}}>
              ...
              <div style={{display:"flex",gap:6}}>
                <button onClick={()=>setAddRouteFor(trip.id)}
                  style={{background:"rgba(79,142,247,.12)",border:"1px solid rgba(79,142,247,.2)",
                    borderRadius:7,padding:"5px 10px",color:"#4F8EF7",fontSize:11,fontWeight:600,
                    cursor:"pointer",fontFamily:"Inter,sans-serif"}}>+ Route</button>
                {trip.routes.length===0&&(
                  <button onClick={()=>delTrip(trip.id)}
                    style={{background:"rgba(248,113,113,.08)",border:"1px solid rgba(248,113,113,.15)",
                      borderRadius:7,padding:"5px 8px",color:"#F87171",cursor:"pointer",display:"flex"}}>
                    <TrashIcon/>
                  </button>
                )}
              </div>
            ...
            {trip.routes.length===0
              ?<div style={{padding:"18px 16px",textAlign:"center",color:"#555E73",fontSize:13}}>
                  ...
                  <SwipeRow key={route.id} onDelete={()=>delRoute(trip.id,route.id)}>
                    <div onClick={()=>setDetail({tripId:trip.id,routeId:route.id})}
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:14,fontWeight:700,color:"#F0F2F8",letterSpacing:-0.3}}>
                          {route.from} <span style={{color:"#555E73",fontWeight:300}}>→</span> {route.to}
                        </div>
                        <div style={{fontSize:11,color:"#555E73",marginTop:2}}>
...
function AlertsScreen({ trips, setTrips }) {
  ...
  const delRoute = (tid,rid) => setTrips(ts=>ts.map(t=>t.id===tid?{...t,routes:t.routes.filter(r=>r.id!==rid)}:t));
  ...
  if (detail) {
    ...
    if (route) return (
      <RouteDetail route={route} tripColor={trip?.color}
        onBack={()=>setDetail(null)}
        onDelete={()=>{ delRoute(detail.tripId,detail.routeId); setDetail(null); }}
        onEdit={()=>{}}/>
    );
  }
  ...
  const Row = ({r,hi}) => (
    <SwipeRow onDelete={()=>delRoute(r.tripId,r.id)}>
      <div onClick={()=>setDetail({tripId:r.tripId,routeId:r.id})}
        ...
        <div style={{width:7,height:7,borderRadius:"50%",flexShrink:0,
          background:hi?"#34D399":"#4F8EF7",
          boxShadow:hi?"0 0 0 3px rgba(52,211,153,.18)":"none"}}/>
        <div style={{flex:1}}>
          <div style={{fontSize:13,fontWeight:600,color:"#F0F2F8"}}>{r.from} → {r.to}</div>
          <div style={{fontSize:11,color:"#555E73",marginTop:2}}>
            {r.tripName} · {fmt(r.depart)}
            {r.flex>0&&<span style={{color:"#FBBF24"}}> ±{r.flex}d</span>}
          </div>
        </div>
        <div style={{textAlign:"right"}}>
        ...
        {hi&&<div style={{background:"rgba(52,211,153,.12)",border:"1px solid rgba(52,211,153,.2)",
          borderRadius:20,padding:"3px 8px",fontSize:10,fontWeight:700,color:"#34D399"}}>↓ Drop</div>}
      </div>
    </SwipeRow>
  );
          ...
          <div key={r.id} style={{background:"#161A22",border:"1px solid #252B3B",borderRadius:14,padding:16}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
              <div>
                ...
                <div style={{fontSize:11,color:"#555E73",marginTop:2}}>
                  {r.tripName} · {fmt(r.depart)}{r.ret?` – ${fmt(r.ret)}` : ""}
                </div>
              </div>
              <div style={{textAlign:"right"}}>
                <div style={{fontSize:18,fontWeight:800,letterSpacing:-1,color:r.currentPrice<r.threshold?"#34D399":"#F0F2F8"}}>${r.currentPrice}</div>
                <div style={{fontSize:11,fontWeight:600,color:trend<0?"#34D399":"#F87171",marginTop:2}}>
                  {trend<0?"↓":"↑"} ${Math.abs(trend)} over 14d
                </div>
              </div>
            </div>
  ...
  return (
    <div style={{padding:"16px 20px",display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div style={{background:"#161A22",border:"1px solid #252B3B",borderRadius:14,overflow:"hidden"}}>
        ...
        {[["2x","2× daily","Morning + Evening"],
          ["3x","3× daily (recommended)","Morning, Afternoon, Evening"],
          ["4x","4× daily","Every 6 hours"]].map(([v,l,s])=>(
          <div key={v} onClick={()=>setFreq(v)}
            style={{display:"flex",alignItems:"center",justifyContent:"space-between",
              padding:"13px 16px",borderTop:"1px solid #252B3B",cursor:"pointer"}}>
            <div>
              <div style={{fontSize:13,fontWeight:600,color:"#F0F2F8"}}>{l}</div>
      ...
      <div style={{background:"#161A22",border:"1px solid #252B3B",borderRadius:14,overflow:"hidden"}}>
        ...
        {trips.map(t=>(
          <div key={t.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",
            padding:"13px 16px",borderTop:"1px solid #252B3B"}}>
            <div style={{display:"flex",alignItems:"center",gap:9}}>
      ...
      <div style={{background:"rgba(52,211,153,.06)",border:"1px solid rgba(52,211,153,.15)",borderRadius:14,padding:16}}>
        ...
        <div style={{fontSize:11,color:"#555E73",lineHeight:1.5}}>
          Your server checks prices {freq==="2x"?"twice":"three times"} a day. Push notifications fire the moment any route drops below your threshold.
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0A0C10",fontFamily:"Inter,sans-serif",display:"flex",flexDirection:"column"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        input::placeholder{color:#555E73;}
      ...
      <div style={{position:"fixed",top:0,left:0,right:0,zIndex:100,
        background:"rgba(10,12,16,.92)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",
        borderBottom:"1px solid #252B3B",padding:"0 20px",height:56,
        display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"center",gap:9,fontWeight:800,fontSize:17,letterSpacing:-0.4,color:"#F0F2F8"}}>
          <div style={{width:30,height:30,background:"linear-gradient(135deg,#4F8EF7,#7C5CF6)",borderRadius:9,
            display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,
            boxShadow:"0 0 14px rgba(79,142,247,.4)"}}>✈</div>
          FlightWatch
        </div>
        <div style={{color:"#8B93A8",position:"relative",cursor:"pointer",padding:6}} onClick={()=>setTab("alerts")}>
          {dropCount>0&&<div style={{position:"absolute",top:2,right:2,width:17,height:17,
            background:"#F87171",borderRadius:"50%",fontSize:9,fontWeight:700,color:"#fff",
      ...
      {/* Scrollable content */}
      <div style={{flex:1,overflowY:"auto",paddingTop:56,paddingBottom:72}}>
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:100,
        ...
        background:"rgba(13,15,20,.96)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",
        paddingBottom:"env(safe-area-inset-bottom,12px)"}}>
        {TABS.map(t=>{
          const on=tab===t.id;
          return (
            <button key={t.id} onClick={()=>setTab(t.id)}
              style={{flex:1,padding:"10px 0 6px",background:"transparent",border:"none",
                cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",
                gap:4,color:on?"#4F8EF7":"#555E73",transition:"color .15s"}}>
              <t.I/>
